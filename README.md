...
- **Yanked to /login out of nowhere while browsing public pages**: a background fetch (boot prefetches, `fetchMe`) hit a 401 and the token refresh failed. Those callers pass `authRedirect: false` so `useApi`'s refresh-failure redirect must not fire for them, and the redirect itself is single-flight — if you see a redirect storm (multiple rapid `/login` pushes), some new background caller is missing the flag.
